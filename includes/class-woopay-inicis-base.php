...
if ( ! class_exists( 'WooPayInicisBase' ) ) {
	class WooPayInicisBase extends WooPayInicis {
		...
		private function woopay_init() {
			...
			add_action( 'add_woopay_inicis_virtual_bank_information', array( $this, 'add_virtual_information' ), 10, 2 );
			add_action( 'woopay_inicis_virtual_bank_schedule', array( $this, 'woopay_cancel_unpaid_virtual_bank_orders' ) );
			...
			// Classes
			include_once( 'class-woopay-inicis-actions.php' );
			include_once( 'class-woopay-inicis-payment.php' );
			include_once( 'class-woopay-inicis-refund.php' );

			// API
			include_once( 'class-woopay-inicis-api.php' );

			// Payment Methods
			include_once( 'methods/class-woopay-inicis-card.php' );
			include_once( 'methods/class-woopay-inicis-transfer.php' );
			include_once( 'methods/class-woopay-inicis-virtual.php' );
			include_once( 'methods/class-woopay-inicis-mobile.php' );
		}
		...
		public function woopay_add_email_class( $emails ) {			
			$emails[ 'WooPayInicisEmail' ] = include( 'class-woopay-inicis-email.php' );
		...
		public function woopay_cancel_unpaid_virtual_bank_orders() {
			...
			$virtual_bank_settings = get_option( 'woocommerce_inicis_virtual_settings', null );
			...
			if ( $unpaid_orders ) {
				foreach ( $unpaid_orders as $unpaid_order ) {
					...
					$expiry_date = get_post_meta( $unpaid_order, '_woopay_inicis_expirydate', true );
		...
		public function for_translation() {
			array(
				__( 'Korean Payment Gateway integrated with Inicis for WooCommerce.', $this->woopay_domain )
			);
		}
	}

	return new WooPayInicisBase();
}