...
class INILog {
    ...
    function StartLog() {
        ...
        if ($this->mergelog == "1") {
            ...
            $logfile = $this->keydir . "/log/" . PROGRAM . "_" . $type . "_mergelog_" . date("ymd") . ".log";
		} else {
            //$logfile = $this->homedir . "/log/" . PROGRAM . "_" . $type . "_" . $this->mid . "_" . date("ymd") . ".log";
			$logfile = $this->keydir . "/log/" . PROGRAM . "_" . $type . "_" . $this->mid . "_" . date("ymd") . ".log";
		}
...
class INICrypto {
    ...
    function LoadPGPubKey(&$pg_pubcert_SN) {
		...
		$fp = fopen($this->keydir . "/key/pgcert.pem", "r");
        if (!$fp)
    ...
    function UpdatePGPubKey($pgpubkey) {
        ...
        $f_org = $this->keydir . "/key/pgcert.pem";
        $f_new = $this->keydir . "/key/.pgcert.pem.tmp";
        $fp = fopen($f_new, "w");
        if (!$fp)
    ...
    function LoadMPubKey(&$m_pubcert_SN) {
        if ($this->mkey == "1") {
            ...
            $fp = fopen($this->keydir . "/key/mkey/mcert.pem", "r");
        } else {
            //$fp = fopen($this->homedir . "/key/" . $this->mid . "/mcert.pem", "r");
			$fp = fopen($this->keydir . "/key/" . $this->mid . "/mcert.pem", "r");
		}
        if (!$fp)
    ...
    function LoadMPrivKey() {
        ...
        if ($this->mkey == "1") {
            ...
            $fp = fopen($this->keydir . "/key/mkey/mpriv.pem", "r");
        } else {
            //$fp = fopen($this->homedir . "/key/" . $this->mid . "/mpriv.pem", "r");
            $fp = fopen($this->keydir . "/key/" . $this->mid . "/mpriv.pem", "r");
		}
        if (!$fp)
